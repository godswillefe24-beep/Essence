...
test("tokenize", async (t) => {
  ...
  await t.test("strips punctuation", () => {
    assert.deepEqual(tokenize("What's JWT-based auth?"), ["what", "jwt", "based", "auth"]);
  });
...
test("scorePostKeyword", async (t) => {
  const post = { title: "Understanding Digital Marketing", excerpt: "A guide to SEO and social media." };

  await t.test("title matches score higher than body matches (3x weight)", () => {
...
test("getRelevantPostsKeyword", async (t) => {
  const posts = [
    { slug: "post9", title: "Understanding Digital Marketing", excerpt: "SEO, social media, email marketing." },
    { slug: "post16", title: "Rare Programming Languages", excerpt: "Brainfuck, Whitespace, Piet." },
  ];

  await t.test("returns the matching post for a relevant query", async () => {
    const results = getRelevantPostsKeyword("tell me about digital marketing", posts);
    assert.equal(results.length, 1);
    assert.equal(results[0].slug, "post9");
  });

  await t.test("returns nothing for a query below the minimum keyword score", () => {
    // A single incidental match (score 1) shouldn't qualify — this is the
    // exact fix for the wrong-post-citation bug found earlier.
    const results = getRelevantPostsKeyword("what about brainfuck specifically", posts);
...
test("getRelevantPostsEmbedding — using an injected fake embedding function", async (t) => {
  ...
  const posts = [
    { slug: "post-a", title: "Post A", excerpt: "About A", embedding: JSON.stringify([1, 0, 0]) },
    { slug: "post-b", title: "Post B", excerpt: "About B", embedding: JSON.stringify([0, 1, 0]) },
    { slug: "post-c", title: "Post C", excerpt: "About C", embedding: JSON.stringify([0.9, 0.1, 0]) },
  ];
  ...
  await t.test("ranks the closest-matching post first", async () => {
    const results = await getRelevantPostsEmbedding("query text", posts, fakeEmbedFn);
  ...
  await t.test("throws when no posts have embeddings yet (caller is expected to catch and fall back to keyword search)", async () => {
    const postsWithoutEmbeddings = [{ slug: "post-x", title: "X", excerpt: "x" }];
    await assert.rejects(() => getRelevantPostsEmbedding("query", postsWithoutEmbeddings, fakeEmbedFn));
  });
});

test("buildMatchedPost", async (t) => {
  await t.test("falls back to the DB excerpt when the static HTML file isn't found", () => {
    const result = buildMatchedPost({ slug: "nonexistent-post", title: "Test Post", excerpt: "Fallback excerpt text" });
...
test("buildSystemPrompt", async (t) => {
  await t.test("includes page content when pageContext is provided", () => {
    const prompt = buildSystemPrompt([], { title: "My Post", url: "/posts/post1.html", content: "The article body text." });
  ...
  await t.test("includes relevant post excerpts when provided", () => {
    const prompt = buildSystemPrompt([{ title: "Related Post", excerpt: "Related content here." }], null);
    assert.match(prompt, /Related Post/);
    assert.match(prompt, /Related content here\./);
  });

  await t.test("includes the 'no content' fallback instruction when nothing is provided", () => {
    const prompt = buildSystemPrompt([], null);
    assert.match(prompt, /say so plainly/);
  });

  await t.test("never claims it can't see the page when pageContext IS provided (regression test for the original bug)", () => {
    const prompt = buildSystemPrompt([], { title: "X", url: "/x", content: "Some real content." });
    assert.doesNotMatch(prompt, /No specific page or post content was provided/);
  });
});
